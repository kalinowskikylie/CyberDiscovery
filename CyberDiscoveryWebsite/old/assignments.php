	...
	foreach ($bloggityExplosion as $b) {
		...
		if ($bpic !== 'undefined') {
			if (isset($curBlog)) {
				...
				$bloggity[$count].='<center><img class="blogpic" src="http://cyberdiscovery.latech.edu/old/assignments/' . $bpic . '" width="745" name="' . $bpic . '"></center>';
				$bloggity[$count].='</div>';
			} else {
				$bloggity[$count].='<div name="bpic" class="fright">';
				$bloggity[$count].='<img class="blogpic" src="http://cyberdiscovery.latech.edu/old/assignments/thumbs/' . $bpic . '" width="200" name="' . $bpic . '">';
				$bloggity[$count].='</div>';
			}
		}
		...
		if (isset($curBlog)) {
			$bloggity[$count].=$bcontent;
                        if(isset($bfile1)){
                             ...
                             $bfile[$count].='<br /><a style="text-decoration:underline;" name="bfile1" href="http://cyberdiscovery.latech.edu/old/assignments/' . $bfile1 . '">' . $bfile1 . '</a>';
                        } else {
                             $bfile[$count].='<a name="bfile1"></a>';
                        }
                        if(isset($bfile2)){
                             $bfile[$count].='<br /><a style="text-decoration:underline;" name="bfile2" href="http://cyberdiscovery.latech.edu/old/assignments/' . $bfile2 . '">' . $bfile2 . '</a>';
                        } else {
                             $bfile[$count].='<a name="bfile2"></a>';
                        }
                        if(isset($bfile3)){
                             $bfile[$count].='<br /><a style="text-decoration:underline;" name="bfile3" href="http://cyberdiscovery.latech.edu/old/assignments/' . $bfile3 . '">' . $bfile3 . '</a>';
                        } else {
...
<?php if(($_SESSION['cdpermissions']>5)){ ?>
    ...
    <div id="adminsection">
        ...
        <?php } else {?>
        	<span class="adminbutton" name="addblog"><a href="#">Add blog</a></span>
                <?php if(($_SESSION['cdpermissions'] == 100)){ ?>
...
<?php 
			if(isset($curBlog)){
				...
				echo('<a href="http://cyberdiscovery.latech.edu/old/assignments.php?curpage='. $landingPage .'">Back</a>');
		...
		<?php if(!isset($curBlog)){ ?>
            ...
            <?php
				...
				for($i=$displayPage; $i>=$displayPageStop; $i=$i-1){
					echo('<a href="http://cyberdiscovery.latech.edu/old/assignments.php?curblog='. $i .'">'. stripslashes($bloggity[$i]).'</a>');
				}
				echo('</div>');
				...
				if ($bloggitynum > 5) {
					if ($curPage == $bloggityPagenum) {
						...
						echo('<div class="fright"><a href="http://cyberdiscovery.latech.edu/old/assignments.php?curpage='. $prevPage .'">PREV</a>&nbsp;&nbsp;&nbsp;');
					}
					
					for($j=$bloggityPagenum; $j>=1; $j=$j-1){
						$desiredPage = $j;
						echo('<a href="http://cyberdiscovery.latech.edu/old/assignments.php?curpage='. $desiredPage .'">'.$j.'</a>&nbsp;');
					}
					if ($curPage == 1) {
						echo('&nbsp;&nbsp;NEXT</div><div style="clear:both;"></div>');
					} else {
						echo('&nbsp;&nbsp;<a href="http://cyberdiscovery.latech.edu/old/assignments.php?curpage='. $nextPage .'">NEXT</a></div><div style="clear:both;"></div>');
					}
				}
                                echo('</div>');
			?>
