$(document).ready(function() {
	...
	$('#addblog').click(function(event) {
		...
		$('#adminsection').html('<a href="blog.php">Nevermind.</a>');
	...
	$('#editblog').click(function(event) {
		...
		$('#blogcontainer').children('div').click(function() {
			...
			$(this).wrapInner('<form action="bin/blog_upload.php" method="post" enctype="multipart/form-data"></form>');
		...
		$('.deleteblogbutton').click(function() {
			...
			if (answer){
				...
				$.ajax({
				   ...
				   success: function(){
					 window.location = "http://cyberdiscovery.latech.edu/blog.php";
				   }
				 });
			}
		});
